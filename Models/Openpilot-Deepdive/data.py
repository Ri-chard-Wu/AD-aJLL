...
# a = glob.glob("./data/comma2k19-LD/scb*")
...
class Comma2k19SequenceDataset(PlanningDataset):
    ...
    def __init__(self, 
            paths,  # 'data/comma2k19_train_non_overlap.txt'.            
            mode,  # 'train'.
            return_origin=False # False.
            ):
 

        # print('Comma2k19SequenceDataset:')         
    ...
    def __getitem__(self, idx): # each item is all frames of an video (.hevc).
        ...
        # print(f'>>>> [{self.mode}] idx: {idx}')

        # eg: './data/comma2k19-LD/scb82'.
        seq_sample_path = self.samples[idx]
        cap = self._get_cv2_vid(seq_sample_path + '/video.hevc') # TODO:iii.
        ...
        input_img = torch.cat((input_img[:-1, ...], input_img[1:, ...]), dim=1)  # [N+1, 6, H, W], 2 rgb, not yuv.

        # positions are in ECEF (earth-centric) frame in meters.
        frame_positions = self._get_numpy(
                    seq_sample_path + '/global_pose/frame_positions' # TODO:iii.
                    )[seq_start_idx: seq_end_idx + self.num_pts]
        
        # rotate ECEF frame into local frame.
        frame_orientations = self._get_numpy(
                seq_sample_path + '/global_pose/frame_orientations' # TODO:iii.
                    )[seq_start_idx: seq_end_idx + self.num_pts]


        # print(f'frame_positions: {frame_positions.shape}')
        # print(f'frame_orientations: {frame_orientations.shape}')
        ...
        rtn_dict = dict(
            seq_input_img=input_img,  # (N+1, 6, h=128, w=256).
            seq_future_poses=future_poses,  # (N, num_pts, 3).
        )
