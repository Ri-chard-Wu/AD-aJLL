...
for b_idx, batch in enumerate(val_loader):
    ...
    # seq_labels: future car positions (start from frame i to frame i+num_pts(=200)) relative to car's local coord sys.
    ...
    for t in tqdm(range(seq_length)):
        ...
        with torch.no_grad():
            ...
            pred_trajectory = pred_trajectory.reshape(planning_v0.M, planning_v0.num_pts, 3).cpu().numpy() # (M, num_pts, 3).

        inputs, labels = inputs.cpu(), labels.cpu() # (1, 6, h, w), (1, num_pts, 3).
        ...
        visualize(origin_imgs[0, t, :, :, :].numpy(), 
             ...
             pred_conf, # (M,).
             save_path='vis/M5_DEMO_%04d/%08d.jpg' % (seq_idx, img_idx)
             )
        ...
        # img_0 = vis_img[..., :3].numpy().astype(np.uint8)        
        # img_1 = vis_img[..., 3:].numpy().astype(np.uint8)
        ...
        # # Obtain homogeneous coord: [x/z, y/z] == [rl/fb, du/fb], then proj with camera intrinsics, then 
        #         # draw on the input img.
