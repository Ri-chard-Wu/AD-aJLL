...
class SequencePlanningNetwork(tf.keras.Model):
    ...
    def extract_feature(self, x):
        ...
        features = self.plan_head(features, training=training) # (b, 1024).
        return features
    ...
    def call(self, x, feature_sequence, training=True):
        '''
            x: (b, 12, 128, 256).
            feature_sequence: (b, T-1, 1024).
        '''

        features = self.extract_feature(x)[:, None, :] # (b, 1, 1024).
        x = tf.concat([features, feature_sequence], axis=1) # (b, T, 1024).

        x = self.temporal_attention(x, training=training) # (b, 1, 1024).

        x = self.plan_head_tip(x, training=training) # (b, M+M*(2*num_pts+1)).

        pred_cls = tf.nn.softmax(x[:, :self.M], axis=-1) # (b, 5).

        pred_traj = tf.reshape(x[:, self.M:],\
