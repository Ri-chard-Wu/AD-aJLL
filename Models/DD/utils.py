...
# trajectories shape: (M+1, num_pts, 3). M+1: number of predicted traj + ground truth.
def draw_trajectory_on_ax(ax: Axes, trajectories, confs, line_type='o-',
                           transparent=True, xlim=(-30, 30), ylim=(0, 100)):
    '''
    ...
    for idx, (trajectory, conf) in enumerate(zip(trajectories, confs)):
        ...
        # trajectory: (num_pts, 3).
        # y is right-left; x is front-back.
        ax.plot(trajectory[:, 1],  # y, # - for nuscenes and + for comma 2k19
                trajectory[:, 0],  # x
                line_type, **plot_args)
...
def get_val_metric(pred_cls, # (M,) np.
                pred_trajectory, # (M, 2*num_pts+1) np.
                labels # (2*num_pts+1) np.
                ):
    ...
    path_pred = pred_trajectory[idx][:num_pts] # (num_pts,).
    path_true = labels[:num_pts] # (num_pts,).
    l2_dists = np.mean((path_pred-path_true)**2)

    pred_trajectory[:, :num_pts] - labels[:num_pts]
    ...
    rtn_dict.update({'l2_dist': l2_dists, 'cls_acc': best_match == pred_label})
...
# device_path: (num_pts, 3).
def draw_path(device_path, img, width=1, height=1.2, fill_color=(128,0,255), line_color=(0,255,0)):
    """
    obtain homogeneous coord: [x/z, y/z] == [rl/fb, du/fb], then proj with camera intrinsics.
    """
    # y: right-left(rl); x: front-back(fb); z: down-up(du).
    ...
    # img_from_device: obtain [x/z, y/z] == [rl/fb, du/fb].
    img_points_norm_l = img_from_device(device_path_l) # (num_pts, 2).
    img_points_norm_r = img_from_device(device_path_r) # (num_pts, 2).


    # denormalize: obtain coords on camera screen with intrinsic matrix.
    img_pts_l = denormalize(img_points_norm_l) # (num_pts, 2).
    img_pts_r = denormalize(img_points_norm_r) # (num_pts, 2).
